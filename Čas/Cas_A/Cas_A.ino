...
const int pinMereni = 6; // Logická 0 aktivuje měření
...
void nacti() {
  ...
  EEPROM.get(10, h);
  EEPROM.get(14, m);
  if (h > 60000) h = 0; // Ošetření prázdné EEPROM
  if (m > 59) m = 0;
...
void loop() {
  if (digitalRead(pinReset) == LOW) {
    celkoveSekundy = 0; uloz(); delay(200);
  }
  ...
  if (tik) {
    ...
    if (digitalRead(pinMereni) == LOW) {
      celkoveSekundy++;
      if (celkoveSekundy % 60 == 0) uloz(); // Zápis každou minutu
    }
    ...
    if (h >= 250) lcd.print(" !!"); else lcd.print("   ");
  }
}