...
// ZDE ZMĚŇ HESLO 
const char HESLO[DELKA_KODU + 1] = "1234"; 

const int PIN_RELE = A4; 
const int DOBA_SEPNUTI = 2000; 
...
void zpracujStisk(char znak) {
  ...
  // SBĚR ČÍSLIC (Filtruje pouze znaky '0' až '9')
  if (indexVstupu < DELKA_KODU && (znak >= '0' && znak <= '9')) {
...
void overHeslo() {
  ...
  if (shodujeSe) {
    ...
    lcd.print(">> OTEVRENO <<");
    lcd.setCursor(0, 1);
    lcd.print("Zamek aktivni");
    
    // Aktivace relé
    digitalWrite(PIN_RELE, HIGH);
    delay(DOBA_SEPNUTI); // 2000 ms
    digitalWrite(PIN_RELE, LOW);
